...
class TimeIntervalHandler:
    ...
    def get_expiry(interval: TimeInterval, interval_value: int | str | None = None) -> timedelta | None:
        ...
        if interval == TimeInterval.TOTAL:
            ...
            if isinstance(interval_value, int):
                return timedelta(days=365 * interval_value * 2)
            return timedelta(days=365 * 2)
        elif interval == TimeInterval.MONTHLY:
            if isinstance(interval_value, int):
                return timedelta(days=30 * interval_value * 2)
            return timedelta(days=60)
        elif interval == TimeInterval.DAILY:
            if isinstance(interval_value, int):
                return timedelta(days=interval_value * 2)
            return timedelta(days=2)
        elif interval == TimeInterval.HOURLY:
            if isinstance(interval_value, int):
                return timedelta(hours=interval_value * 2)
            return timedelta(hours=2)
        elif interval == TimeInterval.MINUTES:
            if isinstance(interval_value, int):
                return timedelta(minutes=interval_value * 2)
            return timedelta(minutes=2)
        elif interval == TimeInterval.SECONDS:
            if isinstance(interval_value, int):
                return timedelta(seconds=interval_value * 2)
